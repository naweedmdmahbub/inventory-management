<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <router-link to="/administrator/subcontractors/create">
        <el-button
          v-permission="['add subcontractor']"
          class="filter-item"
    ...
    >
      ...
      <el-table-column align="center" label="Actions" width="350">
        <template slot-scope="scope">
          <!-- <el-button
            v-permission="['view subcontractor']"
            type="warning"
            size="small"
            icon="el-icon-view"
          ...
          <router-link :to="'/administrator/subcontractors/view/'+scope.row.id">
            <el-button
              v-permission="['view subcontractor']"
              type="warning"
              size="small"
          ...
          <router-link :to="'/administrator/subcontractors/edit/'+scope.row.id">
            <el-button
              v-permission="['update subcontractor']"
              type="primary"
              size="small"
              icon="el-icon-edit"
            >
          ...
          <el-button
            v-permission="['manage subcontractor']"
            type="danger"
            size="small"
            icon="el-icon-delete"
...
const subcontractorResource = new Resource('subcontractors');
...
export default {
  ...
  methods: {
    async getList() {
      ...
      this.loading = true;
      const { data, meta } = await subcontractorResource.list(this.query);
      this.list = data;
    ...
    handleDelete(id, name) {
      this.$confirm('This will permanently delete subcontractor ' + name + '. Continue?', 'Warning', {
        confirmButtonText: 'OK',
        cancelButtonText: 'Cancel',
        type: 'warning',
      }).then(() => {
        subcontractorResource.destroy(id).then(response => {
          this.$message({
            type: 'success',
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['SL', 'Subcontractor ID', 'Name', 'Business Name', 'Contact Person', 'Contact No', 'Address'];
        const filterVal = ['index', 'id', 'full_name', 'business_name', 'contact_person', 'contact_no', 'address'];
        const data = this.formatJson(filterVal, this.list);
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: 'subcontractor-list',
        });
