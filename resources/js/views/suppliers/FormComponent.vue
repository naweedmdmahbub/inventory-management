  ...
  <div class="form-container">
    ...
    >
      <el-form-item :label="$t('user.first_name')" prop="first_name">
        <el-input v-model="supplier.first_name" :disabled="mode === 'view'" />
      </el-form-item>
      <el-form-item :label="$t('user.last_name')" prop="last_name">
        <el-input v-model="supplier.last_name" :disabled="mode === 'view'" />
      </el-form-item>
      <el-form-item :label="$t('user.business_name')" prop="business_name">
        <el-input v-model="supplier.business_name" :disabled="mode === 'view'" />
      </el-form-item>
      <el-form-item :label="$t('user.email')" prop="email">
        <el-input v-model="supplier.email" :disabled="mode === 'view'" />
      </el-form-item>
      <el-form-item :label="$t('user.contact_person')" prop="contact_person">
        <el-input v-model="supplier.contact_person" :disabled="mode === 'view'" />
      </el-form-item>
      <el-form-item :label="$t('user.contact_no')" prop="contact_no">
        <el-input v-model="supplier.contact_no" :disabled="mode === 'view'" />
      </el-form-item>
      <el-form-item :label="$t('user.address')" prop="address">
        <el-input v-model="supplier.address" :disabled="mode === 'view'" />
      </el-form-item>
...
export default {
  ...
  methods: {
    cancelled(){
      this.$router.push('/administrator/suppliers');
    },
    async handleSubmit() {
      if (this.supplier.id !== undefined) {
        axios
          .put('api/suppliers/'+this.supplier.id, this.supplier)
          .then(response => {
            this.$router.push('/administrator/suppliers');
            this.$message({
              type: 'success',
        ...
        axios
          ...
          .then(response => {
            this.$router.push('/administrator/suppliers');
            this.$message({
              message: 'New supplier ' + this.supplier.first_name + ' ' + this.supplier.last_name + ' has been created successfully.',
              type: 'success',
              duration: 5 * 1000,
            });
          })
